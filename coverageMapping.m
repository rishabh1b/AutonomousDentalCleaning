function [pointsForPlotting, intermediatePoints] = coverageMapping(sorted_column_indices, sorted_row_indices, shifting_ind, laser_rad)
...
num_points = size(sorted_column_indices,1);
unvisited_c = sorted_column_indices;
unvisited_r = sorted_row_indices;
dontconsider = false(size(sorted_column_indices,1),1);
...
% delta = [delta;[0 0]];
% quiver(pointsForPlotting(:,1),pointsForPlotting(:,2),delta(:,1),delta(:,2),0, 'm','MaxHeadSize',0.2, 'LineWidth',2)
...
% s = scatter(intermediatePoints(:,1), intermediatePoints(:,2),laser_rad*5, 'filled', 'g');
% alpha(s, 0.2)
